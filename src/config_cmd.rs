...
use crate::dispatch::display::{format_list, format_list_detect, format_show_config};
...
pub fn cmd_config(args: &ConfigArgs, config_arg: Option<&Path>) -> anyhow::Result<()> {
    match args.action.as_deref() {
        ...
        Some("list") => cmd_config_list(config_arg),
        Some(a) => Err(anyhow!("unknown action '{}'", a)),
    }
}

fn cmd_config_list(config_arg: Option<&Path>) -> anyhow::Result<()> {
    match find_config(config_arg) {
        Some(path) => {
            let config = load_config(&path).map_err(|e| anyhow!(e.to_string()))?;
